    ...
    runExecutionQueue() {
        ...
        return _asyncToGenerator(function* () {
            while (_this.executionQueue.length > 0) {
                ...
                const callback = _this.owner.tasks[taskname].callback.bind(_this),
        ...
        return _asyncToGenerator(function* () {
            try {
                ...
                if (task.validate !== undefined && !task.validate(_this2.argv)) {
                    _this2.owner.logger.error(`task validation failed - ${ task.name }`);

                    if (task.help !== undefined) {
                        task.help();
                    }

                    return { error: _this2.owner.errors.task_validation_failed, task: task };
                }
