...
let exitCode = 0; /*global process, console  */
...
if (argValidated.argv.version) {
    ...
    if (argValidated.argv.tasks) {
        ...
        const taskPage = _consultant2.default.createPage('Task Menu', {
            _: {
                type: String,
                label: 'Task',
                description: 'The task to be executed',

                values: Object.keys(_2.default.tasks).map(taskKey => {
                    const task = _2.default.tasks[taskKey];

                    return {
                        name: (0, _alignedString.alignedString)([0, task.name, 35, task.description]),
                        value: task.name,
                        'short': task.name
                    };
                }),
        ...
        taskPage.inquiry().then(result => _2.default.exec(result.argv)).catch(err => {
